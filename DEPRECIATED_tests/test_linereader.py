...
def general_context():
    def generate_type_counts(dict_to_count, types):
        ...
        for t in types:
            inds = [
                int(name.split("_")[1])
                for name in dict_to_count.keys()
                if re.search("^" + t + "_[0-9]+", name)
            ]
            counts[t] = max(inds)
            inds = [
                int(name.split("_")[2])
                for name in dict_to_count.keys()
                if re.search("invalid_" + t + "_[0-9]+", name)
            ]
    ...
    return {
        ...
        "example_counts": line_counts,
        "example_inlines": example_inlines,
    ...
    for key in example_counts.keys():
        if key == "comment":
            for line_no in range(1, example_counts[key] + 1):
                assert linereader.is_comment_line(
                    example_lines[key + "_" + str(line_no)]
                )
        else:
            for line_no in range(1, example_counts[key] + 1):
                assert not linereader.is_comment_line(
                    example_lines[key + "_" + str(line_no)]
                )
...
def test_detect_macro(general_context):
    ...
    for key in example_counts.keys():
        if key == "macro":
            for line_no in range(1, example_counts[key] + 1):
                assert linereader.is_macro_def_line(
                    example_lines[key + "_" + str(line_no)]
                )
        else:
            for line_no in range(1, example_counts[key] + 1):
                assert not linereader.is_macro_def_line(
                    example_lines[key + "_" + str(line_no)]
                )
...
def test_detect_token(general_context):
    ...
    for key in example_counts.keys():
        if key == "token":
            for line_no in range(1, example_counts[key] + 1):
                assert linereader.is_novel_token_line(
                    example_lines[key + "_" + str(line_no)]
                )
        else:
            for line_no in range(1, example_counts[key] + 1):
                assert not linereader.is_novel_token_line(
                    example_lines[key + "_" + str(line_no)]
                )
    ...
    for key in inline_counts.keys():
        if key == "comment":
            for line_no in range(1, inline_counts[key] + 1):
                assert linereader.contains_inline_comment(
                    example_inlines[key + "_" + str(line_no)]
                )
        elif key == "invalid_comment":
            for line_no in range(1, inline_counts[key] + 1):
                assert not linereader.contains_inline_comment(
                    example_inlines[key + "_" + str(line_no)]
                )
...
def test_detect_macro_use(general_context):
    ...
    for key in inline_counts.keys():
        if key == "macro":
            for line_no in range(1, inline_counts[key] + 1):
                assert linereader.contains_macro_use(
                    example_inlines[key + "_" + str(line_no)]
                )
        elif key == "invalid_macro":
            for line_no in range(1, inline_counts[key] + 1):
                assert not linereader.contains_macro_use(
                    example_inlines[key + "_" + str(line_no)]
                )
...
def test_detect_inline_latex(general_context):
    ...
    for key in inline_counts.keys():
        if key == "latex":
            for line_no in range(1, inline_counts[key] + 1):
                assert linereader.contains_latex_use(
                    example_inlines[key + "_" + str(line_no)]
                )
        elif key == "invalid_latex":
            for line_no in range(1, inline_counts[key] + 1):
                assert not linereader.contains_latex_use(
                    example_inlines[key + "_" + str(line_no)]
                )
...
def test_categorize_line(general_context):
    ...
    for key in example_counts.keys():
        if key == "comment":
            for line_no in range(1, example_counts[key] + 1):
                lr = linereader.categorize_line(
                    example_lines[key + "_" + str(line_no)], -1
                )
...
def test_separate_tex_comment():
    ex = [
        (
            "@Major-minor $$block 1$$ and $$ block 2 $$ and # a comment",
            (True, [(13, 24), (29, 42)], True, 47),
        ),
        (
            "@Major-minor $$block 1$$ and $$ block # 2 $$ and # a comment",
            (True, [(13, 24), (29, 44)], True, 49),
        ),
        (
            "@Major-minor $$block 1$$ and $$ block # 2 $$ and no comment",
            (True, [(13, 24), (29, 44)], False, -1),
        ),
        (
            "@Major-minor $$block 1$$ and # a comment $$ block 2 $$",
            (True, [(13, 24)], True, 29),
        ),
        (
            "@Major-minor $$block 1$$ and # a comment $$ block 2 $$ # and a second comment",
            (True, [(13, 24)], True, 29),
        ),
        (
            "@Major-minor $$block # comment 1$$ and $$ block 2 $$",
            (True, [(13, 34), (39, 52)], False, -1),
        ),
        (
            "@Major-minor $$block # comment 1$$ and # a comment and $$ block 2 $$",
            (True, [(13, 34)], True, 39),
        ),
        (
            "@Major-minor # a commented $$block 1$$ and $$ block 2 $$",
            (False, [], True, 13),
        ),
    ]
    ...
    for i in range(0, len(ex)):
        ...
        has_tex, tex, has_comm, comm = LineReturn.determine_macro_latex_overlap(
            ex[i], poss_latex, poss_comments
        )
