...
class TestLineCollection:
    ...
    def test_process_one_line_macro(self):
        ...
        assert lc.defined_macros[confirmed_line.macro_name] == Macro(
            confirmed_line, None
        )
    ...
    def test_process_two_line_comment(self):
        ...
        test_Lines: List[Optional[Line]] = [
            LineReader.process_line(test_lines[0], 0),
    ...
    def test_mult_poss_multiline_details(self):
        test_lines: List[str] = [
            "@Major name",
            " namecont",
            "@Major-minor firstminor",
            "@Major-minor secondminor",
            " secondcont",
        ]
        test_Lines: List[Optional[Line]] = []
        for idx, l in enumerate(test_lines):
    ...
    def test_multiline_multilayer_macro_replace(self):
        ...
        assert len(blocks) == 1
        assert (
            blocks[0].get_content(resolved_macros) == "hello value value 2 hello hello"
        )
