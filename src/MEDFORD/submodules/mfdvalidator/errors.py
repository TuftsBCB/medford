...
class MissingContent(MFDErr):
    ...
    def __init__(self, detailobj):
        ...
        if not isinstance(detailobj, Detail):
            raise ValueError(
                "Attempted to create a MissingContentError without a Detail."
            )
...
class MaxMacroDepthExceeded(MFDErr):
    ...
    def __init__(self, macroobjs: List):
        ...
        message: str = (
            f"Macro {self.macros[0].name} on line {self.lineno_head_each_macro[0]} is 11 references deep in a macro reference chain. (Macro history: %s)"
            % "->".join([m.name for m in self.macros])
        )
        ...
        for i in range(0, len(self.macros)):
            helpmsg = helpmsg + "Lines (%d-%d): (%s) %s\n" % (
                self.lineno_all_2d[i][0],
                self.lineno_all_2d[i][-1],
                self.macros[i].name,
                self.macros[i].get_raw_content(),
            )

        super(MaxMacroDepthExceeded, self).__init__(
            type(self).__name__, message, helpmsg
        )
...
class MissingRequiredField(MFDErr):
    ...
    def __init__(self, block_inp, missing_token: str):
        ...
        if not isinstance(block_inp, Block):
            raise ValueError(
                "Attempted to create a MissingRequiredFieldError without a Block."
            )
        ...
        super(MissingRequiredField, self).__init__(
            type(self).__name__, message, helpmsg
        )
    ...
    def get_head_lineno(self) -> int:
        if self.block is not None:
            ...
            raise ValueError(
                "Attempted to get a head lineno of a {self.__name__} error that does not have a Block."
            )
    ...
    def get_lineno_range(self) -> Tuple[int, int]:
        if self.block is not None:
            ...
            raise ValueError(
                "Attempted to get a lineno range of a {self.__name__} error that does not have a Block."
            )
...
class InvalidValue(MFDErr):
    ...
    def __init__(self, block_inp, incorrect_token: str, content: str):
        ...
        if not isinstance(block_inp, Block):
            raise ValueError(
                "Attempted to create an InvalidValue error without a Block."
            )
...
class AtAtReferencedDoesNotExist(MFDErr):
    ...
    def __init__(self, atat_inp, referenced_name: str, named_blocks: List[str]):
        ...
        if not isinstance(atat_inp, AtAt):
            raise ValueError(
                "Attempted to create an AtAtReferencedDoesNotExist without a Detail."
            )
        ...
        super(AtAtReferencedDoesNotExist, self).__init__(
            type(self).__name__, message, helpmsg
        )
    ...
    def get_head_lineno(self) -> int:
        if self.atat is not None:
            ...
            raise ValueError(
                "Attempted to get a head lineno of a {self.__name__} error that does not have an atat object."
            )
...
class MissingCrossReferenceError(MFDErr):

    parent_major_token: str
    reference_type: str
    ...
    def __init__(self, detail_inp, reference_type: str, referenced_value: str,
                 available_names: List[str]):
        self.errtype = ErrType.SYNTAX  #stops validation

        from MEDFORD.objs.linecollections import Detail

        if not isinstance(detail_inp, Detail):
            raise ValueError("MissingCrossReferenceError requires a Detail object.")

        self.detail: Detail = detail_inp
        self.parent_major_token = "_".join(detail_inp.major_tokens)
        self.reference_type = reference_type
        self.referenced_value = referenced_value
        self.available_names = available_names

        self.lineno_all = detail_inp.get_linenos()
        self.lineno_range = (min(self.lineno_all), max(self.lineno_all))
        self.lineno_head = self.lineno_range[0]

        message = (
            f"Line {self.lineno_head}: @{self.parent_major_token}-{reference_type} "
            f"references '{referenced_value}' but no @{reference_type} with that name exists."
        )

        if available_names:
            helpmsg = (
                f"Available @{reference_type} blocks: {', '.join(available_names)}.\n"
                f"Either add: @{reference_type} {referenced_value}\n"
                f"Or use an existing name."
            )
        else:
            helpmsg = (
                f"No @{reference_type} blocks are defined. Add:\n"
                f"  @{reference_type} {referenced_value}"
            )
