...
class LineCollector:
    ...
    def _ProcessLines(self, lines: List[Line]):
        ...
        for line in lines:
            line_collection, detail_collection = self._check_do_completion(
                False, line, state, line_collection, detail_collection
            )
        ...
        # finish up
        if state != "na":
            _, _ = self._check_do_completion(
                True, None, state, line_collection, detail_collection
            )
        else:
            raise ValueError(
                "Somehow reached completion of _ProcessLines without changing state."
            )
    ...
    def _check_do_completion(
        ...
        if (
            final and (detail_collection is not None) and len(detail_collection) > 0
        ) or (
            state == "detail"
            and not (
                isinstance(line, NovelDetailLine)
                or isinstance(line, ContinueLine)
                or isinstance(line, CommentLine)
            )
        ):
    ...
    def get_flat_blocks(self) -> List[Block]:
        t: List[Dict[str, Block]] = [
            named_blocks for major_token, named_blocks in self.named_blocks.items()
        ]
        out: List[Block] = []
        for name_dict in t:
