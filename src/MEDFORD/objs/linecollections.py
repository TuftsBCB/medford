...
class LineCollection:
    ...
    def validate_atat(
        self, macro_defs: Dict[str, str], named_blocks: List[str]
    ) -> bool:
        """
    ...
    def __eq__(self, other) -> bool:
        ...
        if (
            self.has_macros
            and self.used_macro_names is not None
            and other.used_macro_names is not None
        ):
        ...
        if self.used_macro_names is not None and len(self.used_macro_names) > 0:
            ...
            for m in self.used_macro_names:
                ...
                if isinstance(r, str):
                    ...
                    raise TypeError(
                        f"Unknown type returned from Macro.resolve(): {type(r).__name__}"
                    )
            ...
            if self._n_resolutions == Macro.MAX_DEPTH:
                mfdglobals.validator.add_error(
                    MaxMacroDepthExceeded(self._get_resolution_chain())
                )
                return "ERROR"
...
class Detail(LineCollection):
    ...
    def __init__(
        self, headline: NovelDetailLine, extralines: Optional[List[ContinueLine]] = None
    ):
        """
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, type(self)):
            if (
                self.major_tokens == other.major_tokens
...
class AtAt(Detail):
    ...
    def validate_atat(
        ...
        if referenced_name not in named_blocks:
            mfdglobals.validator.add_error(
                AtAtReferencedDoesNotExist(self, referenced_name, named_blocks)
            )
...
class Block:
    ...
    major_tokens: List[str]
    minor_tokens: Optional[
        List[Tuple[str, Detail]]
    ]  # Technically can have MFD block with nothing but a name
    details: List[Detail]
    head_detail: Detail
        ...
        if len(details) > 0:
            ...
            for idx, detail in enumerate(details[1:]):
                if detail.major_tokens != self.major_tokens:
                    ...
                    raise ValueError(
                        f"Block provided details of multiple major tokens: Block Major is {my_majors_str} while line {idx} has major of {detail_majors_str}."
                    )
                if detail.minor_token is None:
                    raise ValueError(
                        f"Block provided a detail with no minor token past first detail: detail # {idx}."
                    )
    ...
    def validate_atat(
        self, macro_defs: Dict[str, str], named_blocks: Dict[str, "Block"]
    ) -> bool:
        """DEPRECIATED."""
        block_names_only = []
