...
class Dictionizer:
    ...
    def __init__(
        self, macro_dictionary: Dict[str, Macro], name_dictionary: Dict[str, Block]
    ):
        ...
        for n, m in macro_dictionary.items():
            ...
            if not isinstance(res, str):
                raise ValueError(
                    f"Attempting to resolve macro {m.name} resulted in an error return, not a string."
                )
    ...
    def _recurse_majors(
        self,
        cur_parent_dict: Dict[str, List[Dict]],
        cur_block: Block,
        cur_major_ind: int = 0,
    ):
        ...
        if cur_major_ind == len(cur_block.major_tokens) - 1:
            ...
            self._recurse_majors(
                cur_parent_dict[c_mt][-1], cur_block, cur_major_ind + 1
            )
    ...
    def _parse_minors(self, cur_parent_dict: Dict[str, Any], cur_block: Block):
        ...
        if cur_block.minor_tokens is not None:
            for _, (minor, detail) in enumerate(cur_block.minor_tokens):
                ...
                cur_parent_dict[minor].append(
                    (detail, detail.get_content(self.resolved_macros))
                )
