...
class ContentMixin:
    ...
    ) -> Tuple[List[int], List[Tuple[int, int]], List[Tuple[int, int, str]]]:
        new_com = [x - self.offset for x in poss_com]
        new_tex = [(x - self.offset, y - self.offset) for (x, y) in poss_tex]
        new_macro = [
            (x - self.offset, y - self.offset, name) for (x, y, name) in poss_macro
        ]
        return (new_com, new_tex, new_macro)

    def resolve_comm_tex_macro_logic(
        self,
        poss_com: List[int],
        poss_tex: List[Tuple[int, int]],
        poss_macro: List[Tuple[int, int, str]],
    ...
    def recurse_comm_tex_overlap(
        self, poss_com: List[int], poss_tex: List[Tuple[int, int]]
    ...
    def recurse_macro_tex_overlap(
        ...
        if len(poss_macro) == 0:
            ...
            poss_macro[0][0] < poss_tex[0][0]
        ):  # this macro is fine, move on to next MAC
            next_loop_ret = self.recurse_macro_tex_overlap(poss_macro[1:], poss_tex)
            return [poss_macro[0]] + next_loop_ret

        elif (
            poss_tex[0][0] < poss_macro[0][0] < poss_tex[0][1]
        ):  # this macro invalid, move on to next MAC
    ...
    def find_macro_uses(self, poss_macro: List[Tuple[int, int, str]]) -> None:
        ...
        if ind_last >= 0:
            if self.has_tex:
                # iterate thru
                deconv_results = self.recurse_macro_tex_overlap(
                    poss_macro[:ind_last], self.tex_locs
                )
    ...
    def replace_macros(self, macro_defs: Dict[str, str]) -> None:
        ...
        for i in range(0, n_macro_uses):
            ...
            self.line = (
                self.line[: cur_macro_pos[0]]
                + macro_defs[cur_macro_name]
                + self.line[cur_macro_pos[1] :]
            )
...
class MacroLine(ContentMixin, Line):
    ...
    def __eq__(self, other) -> bool:
        if (
            ...
            and self.macro_name == other.macro_name
...
class NovelDetailLine(ContentMixin, Line):
    ...
    def __eq__(self, other) -> bool:
        if (
            type(self) == type(other)
            and self.major_tokens == other.major_tokens
            and self.minor_token == other.minor_token
            and self.raw_content == other.raw_content
        ):
